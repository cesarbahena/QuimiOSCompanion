...
class RegistroConsumos(ABC):
    ...
    def rvos_check(self) -> None:
        ...
        while (
            missing_keys > 0
        ):  # Interacción con el usuario en caso de que no se encuentren datos
            input_ = str(
                input(
                    f'No se encontraron {missing_keys} reactivos {", ".join(missing_keys_list)}. Actualiza el diccionario de reactivos. ¿Continuar? S/N '
                )
            ).upper()
            if input_ == "S":
                self.rvos: dict = read_json(self.rvos_json)[self.__class__.__name__]
...
class ReporteAMS(RegistroConsumos):
    ...
    def read(self) -> ReporteAMS:
        try:  # Leer PDF y almacenar las tablas en una lista de DatFrames
            dfs = tabula.read_pdf(
                self.path, pages=1, stream=True, pandas_options={"columns": self.cols}
            )
            while (
                not dfs
            ):  # Interacción con el usuario en caso de que no se encuentren datos
                input_ = str(
                    input(
                        f"No se encontraron datos en el archivo. ¿Es el archivo correcto? S/N "
                    )
                ).upper()
                if input_ == "S":
                    break
    ...
    def transform(
        self, manual_edits="manual_edits", w="numbers", wo="txtPacientes"
    ) -> ReporteAMS:
        ...
        ):  # Invierte las columnas de pacientes sin ediciones manuales y pacientes con ediciones manuales
    ...
    def manual_edits(self, manual_edits="manual_edits") -> bool:
        ...
        filtered_df = self.df[
            self.df[manual_edits] != 0
        ]  # Filtra solo los reactivos con ediciones manuales
        ...
        summary = [
            f"{manual_edit} {rvo}" for manual_edit, rvo in zip(manual_edits, rvos)
        ]
        ...
        while True:  # Preguntar si se ignoran las ediciones manuales
            input_ = str(
                input(
                    f'Se ignorarán {manual_edits.sum()} ediciones manuales ({", ".join(summary)}) ¿Continuar? S/N: '
                )
            ).upper()
            if input_ == "S":
                return (
                    False  # Si se elije continuar devuelve Falso y termina la función
                )
            if input_ == "N":
                break
        while (
            True
        ):  # Si se elije No, preguntar si se toman en cuenta las ediciones manuales
            input_ = str(
                input("¿Tomar en cuenta las ediciones manuales? S/N: ")
            ).upper()
            if input_ == "S":
...
class Bitacora(RegistroConsumos):
    ...
    def read(self) -> Bitacora:
        for header in range(5):  # Buscar la fila de encabezado
            ...
            self.df = pd.read_excel(self.path, sheet_name=self.tipo, header=header)
    ...
    def get_files(self, folder, file_regex) -> List[tuple[str, float]]:
        ...
        return [
            (file, os.path.getctime(folder + file))
            for file in os.listdir(folder)
            if file.startswith(file_regex)
        ]
    ...
    def get_path(self, folder, file_regex) -> str:
        ...
        if not (files := self.get_files(folder, file_regex)):
            raise FileNotFoundError(f"No descargaste el archivo {file_regex}.xlsx")
...
class Organizador:
    def __init__(
        self,
        registros: List[RegistroConsumos],
        *,
        col_rvos: str = "lblNombreEstudioGrd",
    ) -> None:
